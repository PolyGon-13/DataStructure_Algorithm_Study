...
int bm_match(const char txt[],const char pat[])
{
	...
	while(pt<txt_len)
	{
		...
		// 패턴의 마지막 문자의 중복에 대한 경우는 처음부터 skip 배열 안에 n으로 지정하여, 현재 인덱스+n=다음 검사할 패턴의 마지막 문자위치라는 식을 도입하였는데, 패턴의 자미가 문자 이외에 다른 문자들이 한 번 이상 등장하게 되면, 현재 인덱스+x 위치의 결과로 기존의 패턴위치보다 뒤로 갈 수 있는 경우가 생기기에 이를 방지하기 위함
	}
